...
export function createWorkspaceActivityRoutes(deps: WorkspaceActivityRouteDeps, guardWorkspace: (workspaceId: string, userOrgId: string) => Promise<void>) {
  const app = new Hono<AuthEnv>()

  app.get('/api/workspaces/:id/activity', async (c) => {
  ...
  app.get('/api/workspaces/:id/compliance', async (c) => {
    const user = c.get('user') as AuthUser
    await guardWorkspace(c.req.param('id'), user.org_id)
  ...
  app.patch('/api/workspaces/:id/guardrail-events/:eventId/status', async (c) => {
    const user = c.get('user') as AuthUser
    await guardWorkspace(c.req.param('id'), user.org_id)
