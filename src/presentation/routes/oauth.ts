...
export function createOAuthRoutes(deps: OAuthRouteDeps) {
  ...
  // POST /api/oauth/:pluginId/refresh — refresh expired access token
  oauth.post('/api/oauth/:pluginId/refresh', async (c) => {
    ...
    const refreshToken = await deps.orgRepo.findSetting(orgId, `${pluginId}_refresh_token`)
    if (!refreshToken?.value) return c.json({ error: 'no_refresh_token' }, 400)

    const credentials = await resolveCredentials(orgId, pluginId)
    if (!credentials) return c.json({ error: 'no_credentials' }, 400)
    ...
    try {
      ...
      log.error({ pluginId, error: (err as Error).message }, 'Token refresh error')
      return c.json({ error: 'refresh_error' }, 500)
    }
  })
