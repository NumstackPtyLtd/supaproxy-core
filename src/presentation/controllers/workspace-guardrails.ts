...
export interface WorkspaceGuardrailRouteDeps {
...
export function listGuardrails(deps: WorkspaceGuardrailRouteDeps, guard: GuardFn) {
...
export function enableGuardrail(deps: WorkspaceGuardrailRouteDeps, guard: GuardFn) {
  return async (c: import('hono').Context<AuthEnv>) => {
    const user = c.get('user') as AuthUser
    const workspaceId = c.req.param('id')!
...
export function disableGuardrail(deps: WorkspaceGuardrailRouteDeps, guard: GuardFn) {
  return async (c: import('hono').Context<AuthEnv>) => {
    const user = c.get('user') as AuthUser
    const workspaceId = c.req.param('id')!
    const guardrailId = c.req.param('guardrailId')!
